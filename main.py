...
def flightaware_verisi_cek():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # İlk iki tabloyu al (1. Kalkan, 2. İnen)
            for i, tablo in enumerate(tablolar[:2]):
                ...
                # --- PANDAS İLE GÜZELLEŞTİRME ---
                if veriler:
                    df = pd.DataFrame(veriler)
                    # index=False: Satır numaralarını gizler
                    # justify='left': Başlıkları sola yaslar
                    print(df.to_string(index=False, justify='left'))
                else:
                    print("Bu tabloda veri bulunamadı.")
